...
from ..clients.twitter import TwitterAPIClient


class TwitterAuthManager(BaseAuthManager):
    """Twitter authentication manager using Authlib OAuth1Session for OAuth 1.0a."""

    def __init__(self, consumer_key: str, consumer_secret: str,
                 oauth_token: Optional[str] = None, oauth_secret: Optional[str] = None):
        """
        Initialize Twitter authentication manager.

        Args:
            consumer_key (str): Twitter consumer key
            consumer_secret (str): Twitter consumer secret
            oauth_token (str, optional): Twitter OAuth token (if already obtained)
            oauth_secret (str, optional): Twitter OAuth secret (if already obtained)
        """
        super().__init__()
    ...
    async def authenticate(self) -> bool:
        """
        Authenticate with Twitter API using OAuth 1.0a credentials.
    ...
    async def authorize(self) -> Optional[tuple]:
        """
        Run Twitter OAuth 1.0a authorization flow using Authlib.

        Returns:
            tuple or None: (oauth_token, oauth_secret) if successful, None if failed
        """
        if not self._validate_basic_credentials():
            raise Exception('Twitter consumer key and secret are required for authorization.')
    ...
    def _create_client(self) -> TwitterAPIClient:
        """Create Twitter API client instance."""
        if not self.oauth_token or not self.oauth_secret:
            raise Exception('OAuth tokens are required to create Twitter client')
