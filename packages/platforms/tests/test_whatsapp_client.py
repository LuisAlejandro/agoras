...
def test_whatsapp_client_send_document():
    """Test WhatsAppAPIClient send_document method."""
    client = WhatsAppAPIClient('access_token', 'phone_number_id')
    mock_graph_api = MagicMock()
    mock_graph_api.post_object.return_value = {'messages': [{'id': 'msg-126'}]}
    client.graph_api = mock_graph_api
    client._authenticated = True

    result = client.send_document('+1234567890', 'http://doc.pdf', caption='Document', filename='doc.pdf')

    assert result == {'message_id': 'msg-126', 'status': 'sent'}
    call_data = mock_graph_api.post_object.call_args[1]['data']
    assert call_data['type'] == 'document'
    assert call_data['document']['link'] == 'http://doc.pdf'
    assert call_data['document']['caption'] == 'Document'
    assert call_data['document']['filename'] == 'doc.pdf'


def test_whatsapp_client_send_document_without_caption_filename():
    """Test WhatsAppAPIClient send_document without optional parameters."""
    client = WhatsAppAPIClient('access_token', 'phone_number_id')
    mock_graph_api = MagicMock()
    mock_graph_api.post_object.return_value = {'messages': [{'id': 'msg-126'}]}
    client.graph_api = mock_graph_api
    ...
    result = client.send_document('+1234567890', 'http://doc.pdf')

    assert result == {'message_id': 'msg-126', 'status': 'sent'}
    call_data = mock_graph_api.post_object.call_args[1]['data']
    assert 'caption' not in call_data['document']
    assert 'filename' not in call_data['document']


# Send Audio Tests

def test_whatsapp_client_send_audio():
    """Test WhatsAppAPIClient send_audio method."""
    client = WhatsAppAPIClient('access_token', 'phone_number_id')
    mock_graph_api = MagicMock()
    mock_graph_api.post_object.return_value = {'messages': [{'id': 'msg-127'}]}
    client.graph_api = mock_graph_api
    client._authenticated = True

    result = client.send_audio('+1234567890', 'http://audio.mp3')

    assert result == {'message_id': 'msg-127', 'status': 'sent'}
    call_data = mock_graph_api.post_object.call_args[1]['data']
    assert call_data['type'] == 'audio'
    assert call_data['audio']['link'] == 'http://audio.mp3'


# Send Contact Tests

def test_whatsapp_client_send_contact():
    """Test WhatsAppAPIClient send_contact method."""
    client = WhatsAppAPIClient('access_token', 'phone_number_id')
    mock_graph_api = MagicMock()
    mock_graph_api.post_object.return_value = {'messages': [{'id': 'msg-128'}]}
    client.graph_api = mock_graph_api
    client._authenticated = True

    contacts = [{
        'name': {'formatted_name': 'John Doe', 'first_name': 'John'},
        'phones': [{'phone': '+9876543210', 'type': 'CELL'}]
    }]
    result = client.send_contact('+1234567890', contacts)

    assert result == {'message_id': 'msg-128', 'status': 'sent'}
    call_data = mock_graph_api.post_object.call_args[1]['data']
    assert call_data['type'] == 'contacts'
    assert call_data['contacts'] == contacts


# Send Location Tests

def test_whatsapp_client_send_location():
    """Test WhatsAppAPIClient send_location method."""
    client = WhatsAppAPIClient('access_token', 'phone_number_id')
    mock_graph_api = MagicMock()
    mock_graph_api.post_object.return_value = {'messages': [{'id': 'msg-129'}]}
    client.graph_api = mock_graph_api
    client._authenticated = True

    result = client.send_location('+1234567890', 40.7128, -74.0060, name='New York', address='NYC')

    assert result == {'message_id': 'msg-129', 'status': 'sent'}
    call_data = mock_graph_api.post_object.call_args[1]['data']
    assert call_data['type'] == 'location'
    assert call_data['location']['latitude'] == 40.7128
    assert call_data['location']['longitude'] == -74.0060
    assert call_data['location']['name'] == 'New York'
    assert call_data['location']['address'] == 'NYC'


def test_whatsapp_client_send_location_without_name_address():
    """Test WhatsAppAPIClient send_location without optional parameters."""
    client = WhatsAppAPIClient('access_token', 'phone_number_id')
    mock_graph_api = MagicMock()
    mock_graph_api.post_object.return_value = {'messages': [{'id': 'msg-129'}]}
    client.graph_api = mock_graph_api
    ...
    result = client.send_location('+1234567890', 0.0, 0.0)

    assert result == {'message_id': 'msg-129', 'status': 'sent'}
    call_data = mock_graph_api.post_object.call_args[1]['data']
    assert 'name' not in call_data['location']
    assert 'address' not in call_data['location']
...
def test_whatsapp_client_send_document_not_initialized():
    """Test WhatsAppAPIClient send_document raises error when not initialized."""
    client = WhatsAppAPIClient('access_token', 'phone_number_id')

    with pytest.raises(Exception, match='GraphAPI not initialized'):
        client.send_document('+1234567890', 'http://doc.pdf')


def test_whatsapp_client_send_audio_not_initialized():
    """Test WhatsAppAPIClient send_audio raises error when not initialized."""
    client = WhatsAppAPIClient('access_token', 'phone_number_id')

    with pytest.raises(Exception, match='GraphAPI not initialized'):
        client.send_audio('+1234567890', 'http://audio.mp3')


def test_whatsapp_client_send_contact_not_initialized():
    """Test WhatsAppAPIClient send_contact raises error when not initialized."""
    client = WhatsAppAPIClient('access_token', 'phone_number_id')

    with pytest.raises(Exception, match='GraphAPI not initialized'):
        client.send_contact('+1234567890', [])


def test_whatsapp_client_send_location_not_initialized():
    """Test WhatsAppAPIClient send_location raises error when not initialized."""
    client = WhatsAppAPIClient('access_token', 'phone_number_id')

    with pytest.raises(Exception, match='GraphAPI not initialized'):
        client.send_location('+1234567890', 0.0, 0.0)
