...
def test_find_metatags_charset_meta_wins():
    """A meta-charset declaration decodes the page (BS4 parity)."""
    from unittest.mock import patch, Mock
    from agoras.common.utils import find_metatags

    html = '<html><head><meta charset="windows-1252"></head><body><meta property="og:title" content="caf\xe9"></body></html>'
    mock_response = Mock()
    mock_response.status_code = 200
    mock_response.content = html.encode('windows-1252')
    with patch('agoras.common.utils.requests.get', return_value=mock_response):
        result = find_metatags('https://example.com', ['og:title'])
    assert result == {'og:title': 'caf\xe9'}
